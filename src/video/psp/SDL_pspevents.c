    ...
    if(changed) {
        ...
        */
                SDL_SendKeyboardKey((keys & keymap_psp[i].id) ?
                                    SDL_PRESSED : SDL_RELEASED, (Uint32)(keymap_psp[i].sym), SDL_GetScancodeFromKey(keymap_psp[i].sym));    // Urho3D: raw keyboard code
            }
        }
    }
    ...
    if (irkbd_ready) {
        ...
        SIrKeybScanCodeData *scanData;
            ...
            if(pspIrKeybReadinput(buffer, &length) >= 0) {
                ...
                SDL_SendKeyboardKey((keys & keymap_psp[i].id) ?
                                    SDL_PRESSED : SDL_RELEASED, (Uint32)(keymap[raw]), SDL_GetScancodeFromKey(keymap[raw]);    // Urho3D: raw keyboard code
