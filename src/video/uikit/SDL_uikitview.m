...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    while(touch) {
        if (touch == leftFingerDown) {
            /* send mouse up */
            //SDL_SendMouseButton(NULL, SDL_TOUCH_MOUSEID, SDL_RELEASED, SDL_BUTTON_LEFT);    // Urho3D
            leftFingerDown = nil;
        }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    while (touch) {
        if (touch == leftFingerDown) {
            ...
            /* send moved event */
            //SDL_SendMouseMotion(NULL, SDL_TOUCH_MOUSEID, 0, locationInView.x, locationInView.y);    // Urho3D
        }
...
- (BOOL)textField:(UITextField *)_textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    if ([string length] == 0) {
        ...
        SDL_SendKeyboardKey(SDL_PRESSED, 0, SDL_SCANCODE_BACKSPACE);        // Urho3D: raw keyboard code
        ...
        for (i = 0; i < [string length]; i++) {
            ...
            if (mod & KMOD_SHIFT) {
                ...
                SDL_SendKeyboardKey(SDL_PRESSED, 0, SDL_SCANCODE_LSHIFT);    // Urho3D: raw keyboard code
            }
            /* send a keydown and keyup even for the character */
            SDL_SendKeyboardKey(SDL_PRESSED, 0, code);    // Urho3D
            SDL_SendKeyboardKey(SDL_RELEASED, 0, code);
            if (mod & KMOD_SHIFT) {
                /* If character uses shift, press shift back up */
                SDL_SendKeyboardKey(SDL_RELEASED, 0, SDL_SCANCODE_LSHIFT);    // Urho3D
            }
        }
...
{
    SDL_SendKeyboardKey(SDL_PRESSED, 0, SDL_SCANCODE_RETURN);    // Urho3D
