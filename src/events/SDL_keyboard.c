...
SDL_ResetKeyboard(void)
{
    ...
    for (scancode = 0; scancode < SDL_NUM_SCANCODES; ++scancode) {
        if (keyboard->keystate[scancode] == SDL_PRESSED) {
            SDL_SendKeyboardKey(SDL_RELEASED, 0, scancode);    // Urho3D: raw keyboard code
        }
    }
}
...
int
SDL_SendKeyboardKey(Uint8 state, Uint32 keycode, SDL_Scancode scancode)    // Urho3D: raw keyboard code
{
    SDL_Keyboard *keyboard = &SDL_keyboard;
    int posted;
    Uint16 modstate;
    ...
    if (SDL_GetEventState(type) == SDL_ENABLE) {
        ...
        event.key.keysym.raw = keycode;    // Urho3D: raw keyboard code
